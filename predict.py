...
AGE_MODEL = os.path.join(os.path.dirname(
    __file__), 'dataset/deploy_age.prototxt')  # AgeNet
AGE_PROTO = os.path.join(os.path.dirname(
    __file__), 'dataset/age_net.caffemodel')  # carrega o modelo
MODEL_MEAN_VALUES = (78.4263377603, 87.7689143744,
                     114.895847746)  # valores de entrada
AGE_INTERVALS = ['(0, 2)', '(4, 6)', '(8, 12)', '(15, 20)',
                 '(25, 32)', '(38, 43)', '(48, 53)', '(60, 100)']  # intervalo de idade

FACE_PROTO = os.path.join(os.path.dirname(
    __file__), "dataset/deploy.prototxt.txt")  # carrega o modelo
FACE_MODEL = os.path.join(os.path.dirname(
    __file__), "dataset/res10_300x300_ssd_iter_140000_fp16.caffemodel")  # carrega o modelo
...
def get_faces(frame, confidence_threshold=0.5):  # função que detecta faces
    ...
    for i in range(output.shape[0]):  # percorre todas as linhas do output
        ...
        if confidence > confidence_threshold:  # se a confiança for maior que o threshold
            ...
            box = output[i, 3:7] * np.array([frame.shape[1],
                                            frame.shape[0], frame.shape[1], frame.shape[0]])
            start_x, start_y, end_x, end_y = box.astype(
                int)  # converte as coordenadas para int
            start_x, start_y, end_x, end_y = start_x - \
                10, start_y - 10, end_x + 10, end_y + 10  # adiciona 10px aos limites da face
            # se o início da face for menor que 0, seta o início da face como 0
            start_x = 0 if start_x < 0 else start_x
            # se o início da face for menor que 0, seta o início da face como 0
            start_y = 0 if start_y < 0 else start_y
            # se o fim da face for menor que 0, seta o fim da face como 0
            end_x = 0 if end_x < 0 else end_x
            # se o fim da face for menor que 0, seta o fim da face como 0
            end_y = 0 if end_y < 0 else end_y
            # adiciona a face à lista
